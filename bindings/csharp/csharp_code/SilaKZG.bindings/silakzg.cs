using SilaKZG.Native;
using static SilaKZG.Native.NativeMethods;
using System.Runtime.InteropServices;

namespace SilaKZG;

public sealed unsafe class SilaKZG : IDisposable
{
    ...
    public SilaKZG(bool usePrecomp = true)
    {
        _context = sila_kzg_das_context_new(usePrecomp);
    }
    ...
    public void Dispose()
    {
        if (_context != null)
        {
            sila_kzg_das_context_free(_context);
            _context = null;
        }
    }
    ...
    public unsafe byte[] BlobToKzgCommitment(byte[] blob)
    {
        ...
        {
            CResult result = sila_kzg_blob_to_kzg_commitment(_context, blobPtr, commitmentPtr);
    ...
    public unsafe (Memory<byte>[], Memory<byte>[]) ComputeCellsAndKZGProofs(byte[] blob)
    {
        ...
        fixed (byte** outProofsPtrPtr = outProofsPtrs)
        {
            ...
            CResult result = sila_kzg_compute_cells_and_kzg_proofs(_context, blobPtr, outCellsPtrPtr, outProofsPtrPtr);
            ThrowOnError(result);
        }
    ...
    public unsafe Memory<byte>[] ComputeCells(byte[] blob)
    {
        ...
        fixed (byte** outCellsPtrPtr = outCellsPtrs)
        {
            ...
            CResult result = sila_kzg_compute_cells(_context, blobPtr, outCellsPtrPtr);
            ThrowOnError(result);
        }
    ...
    public bool VerifyCellKZGProofBatch(byte[][] commitments, ulong[] cellIndices, byte[][] cells, byte[][] proofs)
    {
        ...
        fixed (ulong* cellIndicesPtr = cellIndices)
        {
            ...
            CResult result = sila_kzg_verify_cell_kzg_proof_batch(_context, Convert.ToUInt64(commitments.Length), commitmentPtrPtr, Convert.ToUInt64(cellIndices.Length), cellIndicesPtr, Convert.ToUInt64(cells.Length), cellsPtrPtr, Convert.ToUInt64(proofs.Length), proofsPtrPtr, verifiedPtr);
            ThrowOnError(result);
        }
        return verified;
    }
    ...
    public (Memory<byte>[], Memory<byte>[]) RecoverCellsAndKZGProofs(ulong[] cellIds, byte[][] cells)
    {
        ...
        fixed (byte** outProofsPtrPtr = outProofsPtrs)
        {
            ...
            CResult result = sila_kzg_recover_cells_and_proofs(_context, Convert.ToUInt64(numInputCells), inputCellsPtrPtr, Convert.ToUInt64(cellIds.Length), cellIdsPtr, outCellsPtrPtr, outProofsPtrPtr);
            ThrowOnError(result);
        }
    ...
    public unsafe (byte[], byte[]) ComputeKzgProof(byte[] blob, byte[] z)
    {
        ...
        fixed (byte* yPtr = y)
        {
            CResult result = sila_kzg_compute_kzg_proof(_context, blobPtr, zPtr, proofPtr, yPtr);
            ThrowOnError(result);
        }
    ...
    public unsafe byte[] ComputeBlobKzgProof(byte[] blob, byte[] commitment)
    {
        ...
        fixed (byte* proofPtr = proof)
        {
            CResult result = sila_kzg_compute_blob_kzg_proof(_context, blobPtr, commitmentPtr, proofPtr);
            ThrowOnError(result);
        }
    ...
    public unsafe bool VerifyKzgProof(byte[] commitment, byte[] z, byte[] y, byte[] proof)
    {
        ...
        fixed (byte* proofPtr = proof)
        {
            CResult result = sila_kzg_verify_kzg_proof(_context, commitmentPtr, zPtr, yPtr, proofPtr, &verified);
            ThrowOnError(result);
        }
    ...
    public unsafe bool VerifyBlobKzgProof(byte[] blob, byte[] commitment, byte[] proof)
    {
        ...
        fixed (byte* proofPtr = proof)
        {
            CResult result = sila_kzg_verify_blob_kzg_proof(_context, blobPtr, commitmentPtr, proofPtr, &verified);
            ThrowOnError(result);
        }
    ...
    public unsafe bool VerifyBlobKzgProofBatch(byte[][] blobs, byte[][] commitments, byte[][] proofs)
    {
        ...
        fixed (byte** proofPtrPtr = proofPtrs)
        {
            ...
            CResult result = sila_kzg_verify_blob_kzg_proof_batch(_context,
        ...
        switch (result.status)
        {
            case CResultStatus.Err:
                ...
                if (errorMessage != null)
                {
                    ...
                    sila_kzg_free_error_message(result.error_msg);
            ...
            case CResultStatus.Ok:
                return;
            default:
                throw new ApplicationException("SilaKZG returned an unexpected result variant");
        }
    }
